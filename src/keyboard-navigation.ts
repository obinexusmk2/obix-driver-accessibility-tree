...
export function createKeyboardNavigationController(): KeyboardNavigationAPI {
  const navigations = new Map<string, KeyboardNavigationHandle>();
  ...
  return {
    createNavigation(id, config) {
      ...
      function isBackward(key: string): boolean {
        if (orientation === "horizontal" || orientation === "both") {
      ...
      const handle: KeyboardNavigationHandle = {
        activate() {
          if (active) return;
          active = true;
