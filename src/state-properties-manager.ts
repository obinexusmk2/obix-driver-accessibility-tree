...
export type AriaStateChangeHandler = (
  element: Element,
  key: string,
...
export function createAriaStateManager(
  config?: AriaStateManagerConfig
): AriaStateManagerAPI {
  const states = new Map<Element, Map<string, string | boolean | undefined>>();
  const handlers = new Set<AriaStateChangeHandler>();
  const pending: Array<{ element: Element; key: string; value: string | boolean | undefined }> = [];
  let batchScheduled = false;
  const batchMode = config?.batchUpdates ?? false;

  function applyToDOM(element: Element, key: string, value: string | boolean | undefined): void {
  ...
  function notifyHandlers(element: Element, key: string, value: string | boolean | undefined): void {
  ...
  return {
    ...
    removeState(element, key) {
      ...
      if (elementStates) {
        ...
        if (elementStates.size === 0) states.delete(element);
      }
